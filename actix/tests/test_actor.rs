...
mod future_cancellation_bug {
    ...
    impl Actor for TestActor {
        ...
        fn started(&mut self, ctx: &mut Self::Context) {
            ctx.add_stream(Box::pin(stream::repeat(())));
        }
    }

    impl StreamHandler<()> for TestActor {
        fn handle(&mut self, _: (), _: &mut Self::Context) {}
    }
    ...
    impl Handler<TestMsg> for TestActor {
        ...
        fn handle(&mut self, _: TestMsg, ctx: &mut Self::Context) -> Self::Result {
