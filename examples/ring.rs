...
fn main() {
    ...
    let _: Addr<_> = Node::create(move|ctx| {
        ...
        let mut prev_addr: Addr<_> = Node{limit: num_nodes * ntimes, next: first_addr.recipient()}.start();
    ...
    system.run();
    match now.elapsed() {
        Ok(elapsed) => println!("Time taken: {}.{:06}", elapsed.as_secs(), elapsed.subsec_micros()),
